...
function initTeXView(rawData) {
    ...
    // Add wheel event listener to the TeXView element
    teXView.addEventListener('wheel', function(e) {
        e.preventDefault();
        onWheelCallback(e.deltaY);
    }, { passive: false });
    
    // Setup wheel event propagation
    setupWheelEventPropagation();
}
        ...
        if (iframeContent) {
            ...
            if (teXView) {
                ...
                if (renderTeXViewFn) {
                    ...
                    // Add wheel event listener to the TeXView element in the iframe
                    teXView.addEventListener('wheel', function(e) {
                        e.preventDefault();
                        onWheelCallback(e.deltaY);
                    }, { passive: false });
                    
                    // Setup wheel event propagation in the iframe
                    if (typeof iframeContent.setupWheelEventPropagation === 'function') {
                        iframeContent.setupWheelEventPropagation();
                    } else {
                        // If the function doesn't exist in the iframe context, define it
                        iframeContent.setupWheelEventPropagation = setupWheelEventPropagation;
                        iframeContent.setupWheelEventPropagation();
                    }
...
// Ensure all content inside TeXView doesn't interfere with wheel events
function setupWheelEventPropagation() {
    var wheelPassThrough = `
        #TeXView * {
            pointer-events: auto;
            touch-action: none; /* Disable browser handling of touch events */
        }
    `;
    appendStyle(wheelPassThrough);
}
